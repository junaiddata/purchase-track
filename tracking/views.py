...
# # AED conversion rates for API item totals (USD=3.675, EUR=4.34) - for future total_value in AED
...
@login_required
@admin_required
def edit_release(request, pk):
    """Edit expected arrival date (and container info) of a release. Admin only."""
    release = get_object_or_404(Release, pk=pk)
    next_url = request.GET.get('next') or request.POST.get('next')
    fallback = redirect('quotation_detail', pk=release.quotation_item.quotation.pk)

    if release.is_received:
        messages.warning(request, "Cannot edit a release that has already been received.")
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, "Release date updated successfully.")
            if next_url:
                return redirect(next_url)
            return fallback
    else:
    ...
    return render(request, 'tracking/edit_release.html', {
        'form': form,
        'release': release,
        'next_url': next_url,
    })
...
#     """Convert amount to AED (USD=3.675, EUR=4.34, AED=1, GBP=4.65)."""
...
def api_item_totals(request):
    ...
    # --- Future: total_value in AED (uncomment Decimal, AED_RATES, _to_aed and use below) ---
    # in_transit_releases = Release.objects.filter(is_received=False).select_related('quotation_item__item', 'quotation_item__quotation')
    # in_transit_value_aed = defaultdict(Decimal)
    # for r in in_transit_releases:
    #     code = r.quotation_item.item.item_code
    #     qty, rate = r.quantity_released or 0, r.quotation_item.rate or 0
    #     cur = r.quotation_item.quotation.currency or 'USD'
    #     in_transit_value_aed[code] += _to_aed(qty * rate, cur)
    # pending_items_q = QuotationItem.objects.filter(quotation__status='CONFIRMED').select_related('item', 'quotation')
    # pending_value_aed = defaultdict(Decimal)
    # for qi in pending_items_q:
    #     code, qty, rate = qi.item.item_code, qi.balance_to_release or 0, qi.rate or 0
    #     pending_value_aed[code] += _to_aed(qty * rate, qi.quotation.currency or 'USD')
    # payload = [{'itemcode': code, 'totalqty_ordered': in_transit_map.get(code,0)+pending_map.get(code,0),
